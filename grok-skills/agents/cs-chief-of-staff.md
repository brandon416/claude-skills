...
when-to-use: Invoke this C-suite agent when the question sits in this seat. Works on Grok, Codex, Cursor, Hermes, and ChatGPT — do not assume Claude-only tools.
---
...
**Steps:**
1. Load `decisions/company-context.md` via context-engine
...
# context-engine loads decisions/company-context.md
echo ""
echo "Routing decision: [single-advisor or boardroom]"
echo "Decision logged to decisions/decisions/raw/$(date +%Y-%m-%d)-$RANDOM.md"
```
